    ...
    if (req.params.type === "utc") {
      if (new Date(req.body).toISOString() === new Date("9999-12-31T23:59:59.999Z").toISOString()) {
        ...
        utils.send400(
          res,
          next,
          "Did not like the value provided for max datetime in the req " + util.inspect(req.body),
        );
      }
    } else if (req.params.type === "utc7ms") {
      if (
        new Date(req.body).toISOString() === new Date("9999-12-31T23:59:59.9999999Z").toISOString() &&
        req.body.endsWith(".9999999Z")
      ) {
      ...
      utils.send400(
        res,
        next,
        "Please provide a valid datetime type 'utc', " +
  ...
  router.get("/max/:type/:case", function (req, res, next) {
    ...
    if (req.params.case === "lowercase") {
      ...
      utils.send400(
        res,
        next,
        "Please provide a valid case for datetime case " +
          "'uppercase', 'lowercase' and not " +
          util.inspect(req.params.case),
      );
    }
    ...
    if (req.params.type === "utc") {
      if (new Date(req.body).toISOString() === new Date("0001-01-01T00:00:00Z").toISOString()) {
        ...
        utils.send400(
          res,
          next,
          "Did not like the value provided for min datetime in the req " + util.inspect(req.body),
        );
      }
    } else if (req.params.type === "localpositiveoffset") {
  ...
  router.get("/:scenario", function (req, res, next) {
    if (req.params.scenario === "null") {
      ...
      res
        .status(400)
        .send(
          "Request path must contain a valid scenario: " +
