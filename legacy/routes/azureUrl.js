  ...
  router.get("/:subscriptionId/resourcegroups/:resourceGroup", function (req, res, next) {
    ...
    if (!subscriptionId || typeof subscriptionId !== "string") {
      utils.send400(
        res,
        next,
        "The provided subscriptionId " + util.inspect(subscriptionId) + "is not defined or is not of type string",
      );
    } else if (queryParamCount !== 1 || apiVersion !== "2014-04-01-preview") {
      utils.send400(
        res,
        next,
        "The provided api-version " + util.inspect(apiVersion) + 'is not equal to "2014-04-01-preview".',
      );
    } else {
