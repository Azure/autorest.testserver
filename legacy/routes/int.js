    ...
    if (bits === 32) {
      if (parseInt(req.body, 10) !== Math.pow(2, bits - 1) - 1) {
        utils.send400(
          res,
          next,
          "Did not like the value provided for max signed 32 bit in the req " + util.inspect(req.body),
        );
      } else {
      ...
      if (req.body === 9223372036854776000) {
        ...
        utils.send400(
          res,
          next,
          "Did not like the value provided for max signed 64 bit in the req " + util.inspect(req.body),
        );
      }
    } else {
    ...
    if (bits === 32) {
      if (parseInt(req.body, 10) !== -Math.pow(2, bits - 1)) {
        utils.send400(
          res,
          next,
          "Did not like the value provided for min signed 32 bit in the req " + util.inspect(req.body),
        );
      } else {
      ...
      if (req.body === -9223372036854776000) {
        ...
        utils.send400(
          res,
          next,
          "Did not like the value provided for min signed 64 bit in the req " + util.inspect(req.body),
        );
      }
    } else {
      utils.send400(res, next, "Please use either 64 or 32 bits in the req " + util.inspect(req.body));
